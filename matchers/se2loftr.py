...
class SE2LoFTRMatcher(DetectorFreeMatcher):
    def __init__(self, conf: SE2LoFTRConfig, device: Optional[torch.device] = None):
        model, cfg = load_model(conf)
        model = model.eval().to(device)
        log(f"[SE2LoFTR] model loaded weights from {conf.weight_path}")
        log(f"[SE2LoFTR] Use the device ({device})")
        if conf.verification:
            log("[SE2LoFTR] Use pre-verification")
        self.model = model
        self.cfg = cfg
        self.conf = conf
        self.device = device
...
def load_model(conf: SE2LoFTRConfig) -> Tuple[LoFTR, CN]:
