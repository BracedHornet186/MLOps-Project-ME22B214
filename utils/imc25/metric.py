...
def mAA_on_cameras(err, thresholds, n, skip_top_thresholds, to_dec=3):
    ...
    return (
        0
        if numerator == 0
        else numerator / (len(thresholds[skip_top_thresholds:]) * (n - to_dec))
    )
...
def check_data(gt_data, user_data, print_error=False):
    ...
    for dataset in gt_data.keys():
        ...
        if dataset not in user_data.keys():
            if print_error:
                print(f"dataset {dataset} not found in submission")
                warnings.warn(f"dataset {dataset} not found in submission")
                print(user_data)
            return False
        ...
        if len(aux) > 0:
            if print_error:
                print(f"submission dataset {dataset} missing some GT images")
                warnings.warn(f"submission dataset {dataset} missing some GT images")
            return False
...
def tth_from_csv(csv_file):
    ...
    if len(set(n_thresholds)) != 1:
        raise ValueError(
            f"Number of thresholds vary per scene: {list(set(n_thresholds))}"
        )
...
def score(
    *,
    ...
    # In order to evaluate scores on a subset of the test set,
    # filter unused datasets in advance
    print(f"datasets: {list(user_data.keys())}")
    gt_data = {k: v for k, v in gt_data.items() if k in set(user_data.keys())}
    ...
    for dataset in gt_data.keys():
        ...
        for i, gt_scene in enumerate(gt_dataset.keys()):
            ...
            for j, user_scene in enumerate(user_dataset.keys()):
                ...
                m_mask_a = np.sum(
                    [
                        mask[dataset][gt_scene][image]
                        for image in mask[dataset][gt_scene].keys()
                    ]
                )
                m_mask_b = np.sum(
                    [
                        not mask[dataset][gt_scene][image]
                        for image in mask[dataset][gt_scene].keys()
                    ]
                )
                ...
                model = register_by_Horn(
                    u_cameras, g_cameras, np.asarray(ths), inl_cf, strict_cf
                )
                ...
                if (len(model["valid_cams"]) == 0) or (len(good_cams_mask_a) == 0):
                    mAA_mask_a = np.float64(0.0)
                else:
                    mAA_mask_a = mAA_on_cameras(
                        model["err"][good_cams_mask_a[model["valid_cams"]]],
                        ths,
                        m_mask_a,
                        skip_top_thresholds,
                        to_dec * pct,
                ...
                if (len(model["valid_cams"]) == 0) or (len(good_cams_mask_b) == 0):
                    mAA_mask_b = np.float64(0.0)
                else:
                    mAA_mask_b = mAA_on_cameras(
                        model["err"][good_cams_mask_b[model["valid_cams"]]],
                        ths,
                        m_mask_b,
                        skip_top_thresholds,
                        to_dec * (1 - pct),
                ...
                if (len(model["valid_cams"]) == 0) or (len(good_cams_mask_a) == 0):
                    err_row_mask_a.append(np.zeros((0, th_n)))
                else:
                    err_row_mask_a.append(
                        model["err"][good_cams_mask_a[model["valid_cams"]]]
                    )
                ...
                if (len(model["valid_cams"]) == 0) or (len(good_cams_mask_b) == 0):
                    err_row_mask_b.append(np.zeros((0, th_n)))
                else:
                    err_row_mask_b.append(
                        model["err"][good_cams_mask_b[model["valid_cams"]]]
                    )
        ...
        if outlier_idx > -1:
            ...
            best_user_scene_sum_mask_a = np.delete(
                best_user_scene_sum_mask_a, outlier_idx
            )
            ...
            best_user_scene_sum_mask_b = np.delete(
                best_user_scene_sum_mask_b, outlier_idx
            )
        ...
        cluster_score_mask_a = get_clusterness_score(
            best_cluster_mask_a, best_user_scene_sum_mask_a
        )
        cluster_score_mask_b = get_clusterness_score(
            best_cluster_mask_b, best_user_scene_sum_mask_b
        )
    ...
    scene_score_dict_mask_a = (
        ...
        if mask_csv is None
            ...
            dataset: score * 100 for dataset, score in zip(gt_data, stat_score_mask_a)
        }
    )
    scene_score_dict_mask_b = (
        ...
        if mask_csv is None
            ...
            dataset: score * 100 for dataset, score in zip(gt_data, stat_score_mask_b)
        }
    )
