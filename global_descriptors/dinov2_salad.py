...
class DINOv2SALADGlobalDescriptorExtractor(DescriptorExtractor):
    def __init__(self, conf: DINOv2SALADConfig, device: torch.device | None = None):
        self.conf = conf
        self.device = device
        model = dinov2_salad(str(resolve_model_path(conf.weight_path)))
...
class ImageDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        conf: DINOv2SALADConfig,
        image_reader: Callable | None = None,
    ):
        self.image_paths = image_paths
        self.conf = conf
        self.image_reader = image_reader or cv2.imread
        self.transforms = dinov2_salad_input_transform(
            image_size=tuple(conf.image_size)
        )

    @classmethod
    def from_scene(
        cls, scene: Scene, extractor: DINOv2SALADGlobalDescriptorExtractor
    ) -> ImageDataset:
        return ImageDataset(
